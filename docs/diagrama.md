...
A continuación se presenta el diagrama estructural del sistema, modelado para reflejar las relaciones del dominio, la separación de responsabilidades y la implementación de los patrones de diseño (Factory, Decorator y Observer).

```mermaid
classDiagram
    %% Entidades del Dominio (Modelos Principales)
    class Paciente {
        +int Id
        +string Nombre
        +string Apellido
    ...
    Paciente "1" <-- "*" Cita : Pertenece a
    Medico "1" <-- "*" Cita : Asignada a
